		...
		$html = '<div ' . ( ! empty( $args['cssID'] ) ? 'id="' . sanitize_html_class( $args['cssID'] ) . '"' : '' ) . ' class="rpwe-block ' . ( ! empty( $args['css_class'] ) ? '' . sanitize_html_class( $args['css_class'] ) . '' : '' ) . '">';
			...
			$html .= '<ul class="rpwe-ul">';
				...
				while ( $posts->have_posts() ) : $posts->the_post();
					...
					$html .= '<li class="rpwe-li rpwe-clearfix"><div class="post_box">';
						...
						if ( $args['excerpt'] ) :
							$html .= '<div class="rpwe-summary">';
								...
								if ( $args['readmore'] ) :
									$html .= '<a href="' . esc_url( get_permalink() ) . '" class="more-link">' . $args['readmore_text'] . '<img class="rdm_arrow" src="' .$url = plugins_url(). '/recent-posts-widget-extended/assets/images/link-arrow.png" alt="" /></a>';
								endif;
							$html .= '</div>';
						endif;
